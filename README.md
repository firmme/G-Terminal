...
- 可调整宽度的连接导航栏、自定义连接分组；连接名称留空时默认显示主机 IP / 主机名。提示文字与标签使用不同颜色。设置里的「自动隐藏导航栏」把导航栏移出布局，只在鼠标移到左侧半透明的 `>` 条时以悬浮层展开；该模式下顶栏的展开 / 收起按钮改为在「常驻」与「自动隐藏」之间切换。
- 设置里可让程序记住上次关闭时的连接标签，重启后按断开状态还原（不重连、不新建 Shell）；关闭窗口时若有未断开的连接会先确认。菜单里的「断开当前连接 / 重新连接」按当前激活会话的状态给出。
