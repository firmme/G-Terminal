...
当前版本 **0.4.1**。Windows x64 便携版沿用本机 Windows / MSVC 环境验证；macOS 使用隐藏标题栏的原生窗口、用户 `$SHELL` 与系统字体，已在 Apple Silicon 完成编译、测试、界面冒烟与本机打包。Linux 的平台入口保留，尚未实机验证或发布。推荐 Windows 11；ConPTY 要求 Windows 10 1809 或更新版本。
...
- 可调整宽度的连接导航栏、自定义连接分组；连接名称留空时默认显示主机 IP / 主机名。提示文字与标签使用不同颜色。设置里的「自动隐藏导航栏」把导航栏移出布局，只在鼠标移到左侧半透明的 `>` 条时以悬浮层展开；该模式下顶栏的展开 / 收起按钮改为在「常驻」与「自动隐藏」之间切换。保存的连接（SSH / 串口）右键可「复制连接」，副本名为「原名 副本」（重名自动编号），跳板机与转发规则一并复制。
...
- **终端标题**：shell 通过 OSC 0/2 设置的窗口标题会显示在**标签**上（没设置时回退到会话名），系统窗口标题也跟随当前标签。
- **响铃**：会话输出里的 BEL（`\x07`）会让**后台标签**显示一个铃铛图标，切到该标签后消失。
- 活动标签用强调色底 + 描边标出，悬浮标签也用轻度强调底，和普通标签区分明显。
